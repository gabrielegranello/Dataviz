...
date=st.selectbox(
    'Choose date',list(pd.unique(greatbritain_hpi.Date))
    )
...
if not date:
    ...
    vmin=min(greatbritain_hpi[greatbritain_hpi.Date==date]['AveragePrice']);
    vmax=max(greatbritain_hpi[greatbritain_hpi.Date==date]['AveragePrice']);
    greatbritain_hpi[greatbritain_hpi.Date==date].plot(ax=ax,column="AveragePrice",legend=True,cmap='plasma',norm=colors.LogNorm(vmin=vmin, vmax=vmax)); # extract the dataset for that given date
